...
int set_random_range(float min, float max) {
    return ((int) (min + max * rand() / (0x7fff)));
}
...
TaskManager_t TaskManager[3];

void TaskManagerInit(void) {
    totalTask = 0;
    memset(&TaskManager[0], ((void*)0), sizeof (TaskManager));
}
    ...
    if (currentMillis - lastTaskTick >0) {
        for (uint8_t i = 0; i < 3; i++) {
            ...
            if ( millis() - TaskManager[i].lastTick > TaskManager[i].interval) {
                ...
                if (TaskManager[i]._callback != ((void*)0)) {
...
void main(void) {
    ...
    for (i = 0; i < 3; i++) {
        ...
        uint8_t k = set_random_range(1.0, 3);
    ...
    qsort(TaskManager, 3, sizeof (TaskManager_t), comparator);
