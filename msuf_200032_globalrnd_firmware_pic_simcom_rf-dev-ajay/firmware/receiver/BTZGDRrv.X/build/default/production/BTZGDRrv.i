...
_Bool decrypt(uint8_t *plain_msg, uint8_t *cypher_msg, uint8_t plain_len, uint8_t cypher_len);
...
typedef struct {
    transmitter_id_t transmitter[(4) + 1];
    cfg_pulse_width_t pulse_width[(4)];
...
_Bool millis_attach_external_callback(external_tick_cb_t cb);
...
_Bool eep_forget_all(void);
int eep_check_tx_id(uint32_t id);
# 1 "BTZGDRrv.c" 2


static uint8_t recv_counter_value[(sizeof(counter_t))];
# 12 "BTZGDRrv.c"
void counter_inc(counter_t *ctx) {
    ctx->word0++;
    if (ctx->word0 == 0) {
...
static _Bool system_cfg_update = 0;
...
void system_config_begin(void) {
    ...
    for (n = 0; n < (4); n++) {
        ...
        if (system_cfg.pulse_width[n].pulse_width.val == (0xFFFFFFFF)) {
            system_cfg.pulse_width[n].pulse_width.val = (3000);
            system_cfg_update = 1;
        }
    }
    ...
    if (system_cfg.transmitter[4].id != (0x55665566)) {
        system_cfg.transmitter[4].id = (0x55665566);
        system_cfg_update = 1;
    }

    if (system_cfg.system_block_timeout_ms > (5U * 60U * 1000UL)) {
...
static external_tick_cb_t external_callback = ((void*)0);
...
static void ra0_ioc_handler(void) {
    ...
    if (released) {
        ...
        if ((uint32_t) (recv_switch.duration) >= 150) {
            recv_switch.event = 1;
        } else {
            recv_switch.event = 0;
        }
...
static volatile relay_task_t relay_tasker[(4)] = {0};
...
void relay_forget_pulse_time(relay_t relay) {
    ...
    relay_set_pulse_time(relay, (3000));
}
...
void relay_set_value(relay_t relay, _Bool value) {
    switch (relay) {
        case RELAY_01:
        {
            if (value) {
...
_Bool relay_get_value(uint8_t relay) {
    _Bool state = 0;
    ...
    for (uint8_t i = 0; i < (4); i++) {
        if (relay_tasker[i].pending) {
            if ((millis()>=relay_tasker[i].start_time) && ((uint32_t )(millis() - relay_tasker[i].start_time)<=( relay_tasker[i].timeout))) {
                ...
                relay_tasker[i].wait = 0;
            } else if (relay_tasker[i].wait == 0) {
                relay_set_value(i, 0);
                relay_tasker[i].pending = 0;
            } else {
            }
        }
    }
...
void relay_pulse_on(relay_t relay, int manual_tout) {
    if (relay < (4)) {
        ...
        relay_tasker[relay].wait = 0;
        if (relay_tasker[relay].timeout > 0 && relay_tasker[relay].timeout < (0xFFFFFFFF)) {

            relay_tasker[relay].pending = 1;
        }
    }
}
...
void relay_trigger_on() {
    ...
    relay_tasker[1].start_time = millis() + ((wait_time == (0xFFFFFFFF)) ? 2000 : wait_time);
    relay_tasker[1].timeout = relay_get_trigger_width();
    relay_tasker[1].wait = 1;
    relay_tasker[1].pending = 1;
}
...
static volatile uint8_t packet_buffer[(74)];
static uint8_t decrypt_packet[(31)];
static volatile uint8_t byte_count;
static volatile _Bool packet_found;
static volatile _Bool packet_received;
...
_Bool rf_receiver_available(void) {
    if (packet_received) {
        ...
        uint8_t crc = crc8(decrypt_packet, (31) - 1);

        if (crc == decrypt_packet[(31) - 1]) {
            return 1;
        }
        packet_received = 0;
    }
...
# 1978 "BTZGDRrv.c"
int rf_receiver_get_msg(uint8_t *buffer, uint8_t blen) {
    if (blen >= (((31)-((04)+(01)+1+(sizeof(counter_t)) + 4)))) {
        uint8_t count = decrypt_packet[25];

        count = (count > (((31)-((04)+(01)+1+(sizeof(counter_t)) + 4)))) ? (((31)-((04)+(01)+1+(sizeof(counter_t)) + 4))) : count;


        memcpy(recv_counter_value, &decrypt_packet[0], sizeof (recv_counter_value));

        memcpy(buffer, &decrypt_packet[25], (((31)-((04)+(01)+1+(sizeof(counter_t)) + 4))));
...
static uint8_t rotary_swtich_update(void) {
    ...
    pos |= (~(PORTBbits.RB3 << 0))&0x01;
    pos |= (~(PORTBbits.RB2 << 1))&0x02;
    pos |= (~(PORTBbits.RB1 << 2))&0x04;
    pos |= (~(PORTBbits.RB0 << 3))&0x08;
...
void eep_erase(void) {
    ...
    eeprom_write_nbytes((0x0000), (uint8_t*) & cfg, sizeof (cfg));
}
...
int eep_remember_tx_id(transmitter_id_t *id) {
    ...
    if (tx_idx >= 0 && tx_idx <= 3) {
        system_cfg.transmitter[tx_idx].id = (0xFFFFFFFF);
    }


    uint8_t n = 0;
    for (n = 0; n < (4); n++) {
        if (system_cfg.transmitter[n].id == (0xFFFFFFFF)) {
...
int eep_forget_tx_id(uint32_t id) {
    ...
    for (n = 0; n < (4); n++) {
        if (system_cfg.transmitter[n].id == id) {
            system_cfg.transmitter[n].id = (0xFFFFFFFF);
            system_cfg.transmitter[n].key = (0xFFFFFFFF);
...
uint8_t msg_buffer[(((31)-((04)+(01)+1+(sizeof(counter_t)) + 4)))] = {0};
...
static _Bool air_learning_id_enroll_done = 0;
static _Bool air_learning_enabled = 0;

static _Bool indicate_after_task = 0;
...
_Bool is_key_pressed(uint32_t hold_time) {
...
_Bool rf_available = 0;
_Bool tx_is_master = 0;
int tx_index = -3;
uint32_t pressed_time = 0;
transmitter_id_t tx;
static _Bool login_success = 0;

uint32_t last_rf_available = 0;
uint32_t last_tx_id = 0;
static int user_retry_count = (3);
...
# 2487 "BTZGDRrv.c"
        if (system_get_user_pin() != (0xFFFFFFFF)) {
            ...
            if (is_key_pressed(pressed_time)) {
                if (data.count < 4) {
                    ...
                    if (system_get_user_pin() == data.u32pass) {
                        ...
                        printf("User pin correct... key is %02X\r\n", data.key);
                        login_success = 1;
                        memset(&data, 0, sizeof (data));
                        user_retry_count = (3);
                        return msg_buffer[0];
                    } else {
                        printf("User pin incorrect... key is %02X\r\n", data.key);
                        login_success = 0;
                        memset(&data, 0, sizeof (data));
                        user_retry_count--;
                        printf("Retry count %d\r\n", user_retry_count);
                        if (user_retry_count <= 0) {
                            user_blocked_timer = (5U * 60U * 1000UL);
                            system_set_block_timeout((5U * 60U * 1000UL));
                        }
                        return 0;
                    }
                }
                ...
                printf("Else %u\r\n", login_success);
                if (pressed_time > 195 && login_success == 1) {
                    printf(" Alive key is %02X\r\n", msg_buffer[0]);
                    data.last_session = millis();
                    return msg_buffer[0];
                }
            }
...
_Bool verify_counter_value() {
    ...
    if ((last_tx_id != tx.id) || (millis() - last_rf_available > 300)) {
        ...
        uint8_t temp_counter_u8[(sizeof(counter_t))];
        ...
        printf("Recv Counter: ");
        for (uint8_t i = 0; i<sizeof (counter_t); i++)printf("%02X ", recv_counter_value[i]);
        printf("\r\n");
        ...
        for (uint32_t i = 0; i < 100; i++) {
            ...
            if (memcmp(temp_counter_u8, recv_counter_value, sizeof (temp_counter_u8)) == 0) {
                ...
                counter_inc_by_n(temp_counter, 5);

                printf("Checking %0d: ", i);
                for (uint8_t i = 0; i<sizeof (counter_t); i++)printf("%02X ", temp_counter_u8[i]);
                printf("\r\n");
    ...
    if (rf_available > 0) {
        ...
        if (tx_index >= 0) {
            ...
            if (tx_index >= 0) {
                ...
                if (system_cfg.transmitter[tx_index].key != (0xFFFFFFFF)) {
                ...
                printf("[Not authorized] Transmitter id :0x%lx\r\n", tx.id);
            }
        }
    }
}
...
void main(void) {
    ...
    printf("\r\n********* Parameter *********\r\n");
    printf("> System user pin            :0x%lX\r\n", system_get_user_pin());
    printf("> System technician pin      :0x%lX\r\n", system_get_technician_pin());
    printf("> Registered Transmitter 0   :0x%lX\r\n", system_cfg.transmitter[0].id);
    printf("> Registered Transmitter 1   :0x%lX\r\n", system_cfg.transmitter[1].id);
    printf("> Registered Transmitter 2   :0x%lX\r\n", system_cfg.transmitter[2].id);
    printf("> Registered Transmitter 3   :0x%lX\r\n\r\n", system_cfg.transmitter[3].id);

    printf("> Relay1 Trigger Wait        :0x%lx\r\n", relay_get_trigger_wait());
    printf("> Relay1 Trigger Width       :0x%lx\r\n", relay_get_trigger_width());
    printf("> Relay1 Trigger Edge        :0x%x\r\n\r\n", relay_get_trigger_edge());

    printf("> Relay1 Activation Mode     :0x%02x\r\n", relay_get_mode(RELAY_01));
    printf("> Relay2 Activation Mode     :0x%02x\r\n", relay_get_mode(RELAY_02));
    printf("> Relay3 Activation Mode     :0x%02x\r\n", relay_get_mode(RELAY_03));
    printf("> Relay4 Activation Mode     :0x%02x\r\n\r\n", relay_get_mode(RELAY_04));
    printf("****************************\r\n");
    ...
    user_retry_count = (3);
    _Bool test_mode = 0;
    device_locked = ((system_get_technician_pin() == (0xFFFFFFFF)) ? 0 : 1);
    user_blocked_timer = system_get_block_timeout();

    printf("System block timeout %lu\r\n", user_blocked_timer);
    while (1) {
        ...
        if (user_blocked_timer <= 0) {
            ...
            if ((rf_available == 1) && ((position > ROT_POS_3) || ((position > ROT_POS_3)))) {
            ...
            if (air_learning_enabled) {
                if (position != ROT_POS_0) air_learning_enabled = 0;

                if ((uint32_t) (millis() - air_learning_time_stamp) <= (15UL * (1000U) )) {
                    ...
                    if (rf_available) {
                        if (tx_index >= 0) {
                            ...
                            air_learning_enabled = 0;
                            delay_ms(3000);
                            printf("Transmitter id :0x%lx Added rc:%d\r\n", tx.id, err);
                            printf("exiting from air learning mode : device enrolled\r\n");
                        }
                    }
                } else {
                    air_learning_enabled = 0;
                    printf("exiting from air learning mode : timeout\r\n");
                }
                ...
                if ((device_locked == 1) && (tx_is_master == 0)) {
                ...
                switch (access_case) {
                    case ROT_POS_0:
                    {
                        ...
                        if (PORTAbits.RA0 == 0U) {
                            ...
                            if ((device_locked == 1) && (tx_is_master == 0)) {
                                ...
                                printf("Device locked...Not allowed");
                                ...
                                if (pressed_time >= 3000) {
                                    ...
                                    if (rf_available) {
                                        if (tx_index >= 0) {
                                            blink_led_async(LED_GREEN, -1, 3000);
                                            delay_ms(3000);
                                            printf("Transmitter id :0x%lx Already exist\r\n", tx.id);
                                        } else {
                                            memcpy(tx.counter, recv_counter_value, sizeof (recv_counter_value));
                            ...
                            if (rf_available) {
                                if ((PORTAbits.RA1) == 0) {
                                    ...
                                    if (tx_index >= 0 && (msg_buffer[0] == 0xAA)) {
                                        printf("Enter into air-learning mode..\r\n");
                                        air_learning_time_stamp = millis();
                                        air_learning_enabled = 1;
                                        ...
                                        printf("invalid code for air learning...\r\n");
                                    }
                    ...
                    case ROT_POS_2:
                    {
                        ...
                        if (rf_available) {
                            if (tx_index >= 0) {
                                ...
                                printf("Forgetting Transmitter id :0x%lx rc :%d\r\n", tx.id, err);
                                blink_led_async_stop(LED_YELLOW);
                                ...
                                printf("Transmitter id :0x%lx Not exist.\r\n", tx.id);
                            }
                        }
                    }
                        break;
                    ...
                    case ROT_POS_4:
                    {
                        if (rf_available && tx_index >= 0) {
                            ...
                            uint8_t jmp_state = 0;
                            jmp_state = ((PORTAbits.RA3) == 0 ? 1 : 0);
                            jmp_state = (uint8_t) (jmp_state << 1);
                            jmp_state |= ((PORTAbits.RA2) == 0 ? 1 : 0);

                            switch (jmp_state) {
                                case 0b00:
                                {

                                    printf("> Setting pulse mode with pulse time => Relay#%d =>%u ms\r\n", msg_buffer[0] - 0xA0, pressed_time);
                                ...
                                case 0b01:
                                {
                                    ...
                                    printf(" Setting Mode Latch0 for Relay#%d\r\n", msg_buffer[0] - 0xA0);
                                ...
                                case 0b10:
                                {
                                    ...
                                    if ((msg_buffer[0] == (0xAA)) || (msg_buffer[0] - 0xA0) <= 0x02) {
                                        relay_set_mode(RELAY_01, RLY_LATCH1_MODE);
                                        relay_set_mode(RELAY_02, RLY_LATCH1_MODE);
                                        printf(" Setting Mode Latch1 for Relay#0\r\n");
                                    }


                                    if ((msg_buffer[0] == (0xAB)) || (((msg_buffer[0] - 0xA0) >= 0x03) && (msg_buffer[0] - 0xA0) <= 0x04)) {
                                        relay_set_mode(RELAY_03, RLY_LATCH1_MODE);
                                        relay_set_mode(RELAY_04, RLY_LATCH1_MODE);
                                        printf(" Setting Mode Latch1 for Relay#1\r\n");
                                    }
                                ...
                                case 0b11:
                                {
                                    ...
                                    printf(" Setting Mode Momentary for Relay#%d\r\n", msg_buffer[0] - 0xA0);
                                }
                                    break;
                            }
                        ...
                        if (PORTAbits.RA0 == 0U) {
                            ...
                            if (receiver_switch->pressed_time > 0) {
                                ...
                                if (pressed_time >= 5000) {
                                    printf("Pressed for %u \r\n", pressed_time);
                                    printf("Forgetting all transmitter...\r\n");
                                    eep_forget_all();
                                    if (task) {
                                        blink_led_async_stop(LED_YELLOW);
                                        task = 0;
                            ...
                            if (tx_index >= 0) {
                                ...
                                switch (msg_buffer[0]) {
                                    case BUTTON_1:
                                    {
                                        ...
                                        printf("> Setting Trigger wait time. =>%u ms\r\n", pressed_time);
                                        break;
                                    }
                                    case BUTTON_2:
                                    {
                                        relay_set_trigger_width(pressed_time);
                                        printf("> Setting Trigger width time =>%u ms\r\n", pressed_time);
                                        break;
                                    }
                                    case BUTTON_3:
                                    {
                                        relay_set_trigger_edge(TRIGGER_ACTIVE_LOW);
                                        printf("> Setting Trigger active low\r\n");
                                        break;
                                    }
                                    case BUTTON_4:
                                    {
                                        relay_set_trigger_edge(TRIGGER_ACTIVE_HIGH);
                                        printf("> Setting Trigger active High\r\n");
                                        break;
                                    }
                                }
                            }
                    ...
                    case ROT_POS_7:
                    {
                        ...
                        if (rf_available && tx_index >= 0) {
                            ...
                            if ((tx_is_master == 1) && (msg_buffer[0] == (0xAA))) {

                                printf("> Master mode...\r\n");
                                printf("> Disabled learning lock...\r\n");
                                system_set_technician_pin((0xFFFFFFFF));
                                blink_led_async(LED_GREEN, 100, 3000);
                                device_locked = 0;
                                delay_ms(3000);
                                ...
                                if (is_key_pressed(pressed_time)) {
                                    ...
                                    if (signup.count >= 4) {
                                        ...
                                        if (PORTAbits.RA0 == 0U) {
                                            if (login_retry > 0) {
                                                if (system_get_technician_pin() == (0xFFFFFFFF)) {



                                                    printf("> Setting learning lock...\r\n");
                                                    ...
                                                    if (signup.u32pass == system_get_technician_pin()) {
                                                        ...
                                                        login_retry--;
                                                        printf("> Incorrect technician code..\r\n");
                                                        printf("> Retry count.. %d\r\n", login_retry);
                                                    }
                                                    login_stamp = millis();
                                                }
                                            } else {
                                                printf("> Login blocked for [%u] seconds\r\n", (uint32_t) ((5U * 60U * 1000UL) - (millis() - login_stamp)));
                                            }
                                        } else {
                                            ...
                                            printf("> User pin: %02x %02x %02x %02x\r\n", signup.password[0], signup.password[1], signup.password[2], signup.password[3]);
                            ...
                            if (signup.apply) {
                                if (millis() - signup.last_session >= 5000) {
                                    printf("> User pin set timeout occur\r\n");
                                    memset(&signup, 0, sizeof (signup));
                                }
                            }
                        }
                    ...
                    case ROT_POS_8:
                    {
                        if (PORTAbits.RA0 == 0U) {
                            ...
                            if (receiver_switch->pressed_time > 0) {
                                ...
                                if (pressed_time >= 3000) {
                                    printf("Pressed for %u \r\n", pressed_time);
                                    printf("Forgetting user pin...\r\n");

                                    if (system_get_user_pin() != (0xFFFFFFFF)) {
                                        system_set_user_pin((0xFFFFFFFF));
                                        blink_led_async(LED_RED, 100, 3000);
                                    } else {
                                        blink_led_async(LED_RED, -1, 3000);
                                    }
                                ...
                                if (tx_index >= 0) {
                                    if (pressed_time >= 3000 && pressed_time <= 3300) {
                                        ...
                                        switch (msg_buffer[0]) {
                                            case BUTTON_1:
                                            {
                                                printf("Forgot pulse time Relay#%d\r\n", msg_buffer[0] - 0xA0);
            ...
            if (login_retry == 0) {
                if ((uint32_t) (millis() - login_stamp) >= (5U * 60U * 1000UL)) {
                    login_retry = (3);
                }
            }
            ...
            delay_ms((30000UL));
            user_retry_count = (3);
            user_blocked_timer = user_blocked_timer - (30000UL);
            if (user_blocked_timer < 1000)user_blocked_timer = 0;
            system_set_block_timeout(user_blocked_timer);
        }

        if (test_mode) {
            printf("Turn rotaty to 1 for DS 1 Mode \r\n");
            while (!position) {
                position = rotary_position_now();
                _Bool task_done = 0;
                while (position==1) {
                    if((PORTAbits.RA1) == 0 && (PORTAbits.RA2) == 0){
                        printf("DS 1 Mode Set. \r\n");
                        printf("Pressed  Push Button and Press  the button on the transmitter for activate Relay #0 \r\n ");
                        printf("if you only Pressed button on Transmitter it will Remember the transmitter ID.\r\n");
                ...
                while (task_done) {
                     if (test_mode) {
                     printf("Turn rotaty to 2 for DS 2 Mode \r\n");
                     while (!(position == 2)) {
                            position = rotary_position_now();
                            while (position == 2) {
                                if ((PORTAbits.RA1) == 0 && (PORTAbits.RA2) == 0) {
                                    printf("WE are in 2nd Mode \r\n");
                                } else {
                                    test_mode = 0;
                                    task_done = 0;
                                    position = ROT_POS_INVALID;
